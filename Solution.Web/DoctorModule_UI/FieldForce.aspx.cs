...
public partial class DoctorModule_UI_FieldForce : System.Web.UI.Page
{
    ...
    [WebMethod]
    public static string GetMBEList()
    {
        string param = " ";

        DataTable dt = _setupDAL.Get_MBEInfo(param);
        string JSONresult;
        JSONresult = JsonConvert.SerializeObject(dt);
        return JSONresult;
    }
    ...
    [WebMethod]
    public static ResultInfo Save_MBEInfo(MBEInfoDao aMIOInfo)
    {
        return (_setupDAL.Save_MBEInfo(aMIOInfo, Convert.ToInt32(HttpContext.Current.Session["UserId"].ToString())));
    }


    [WebMethod]
    public static MBEInfoDao GeEMBEditData(int id)
    {
        return (_setupDAL.GetEMBEditDataDAL(id));
    }
