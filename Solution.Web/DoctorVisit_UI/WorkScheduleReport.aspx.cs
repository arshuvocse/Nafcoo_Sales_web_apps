...
public partial class DoctorVisit_UI_NewDCRReport : System.Web.UI.Page
{
    ...
    {
        DataTable aDataTable = _DoctorVisit_DAL.GetWorkScheduleReport(param());
    ...
    private string param()
    {
        ...
        {
            param = param + " AND CONVERT(date,B.DCPDate)  BETWEEN '" + FromDate.Text + "' AND '" + ToDate.Text + "' ";
        }
        if (FromDate.Text != "" && ToDate.Text == "")
        {
            param = param + " AND CONVERT(date,B.DCPDate)  BETWEEN '" + FromDate.Text + "' AND '" + DateTime.Now.ToString("dd-MMM-yyyy") + "' ";
        }
