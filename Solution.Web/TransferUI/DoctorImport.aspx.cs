...
public partial class TransferUI_DoctorImport : System.Web.UI.Page
{
    ...
    {
        Response.Redirect("DoctorUpdationApprove.aspx");
    }
    protected void cancelButton_Click(object sender, EventArgs e)
    {
        Response.Redirect("DoctorImport.aspx");
    }
    ...
    private void InitialGrid()
    {
        ...
        aDataTable.Columns.Add("DoctorCode");
        aDataTable.Columns.Add("ProviderTypeCode");
        aDataTable.Columns.Add("DoctorTypeCode");
        ...
        dataRow["DoctorCode"] = "";
        dataRow["ProviderTypeCode"] = "";
        dataRow["DoctorTypeCode"] = "";
        dataRow["MarketCode"] = "";
    ...
    private void GetProductInGrid(int rowindex, string productCode)
    {
        ...
        {
            aDataTable = _aDcStockReceiveBll.ProductInfo(productCode,null);
    ...
    private void SaveAllData()
    {
        ...
        if (hiddenField.Value=="")
        {
            Requesition aRequesition = new Requesition()
            {
                                           ...
                                           WarehouseId = Convert.ToInt32(null),
                                           WearhouseName = null,
                                           ComUnitId = Convert.ToInt32(null),
                                           ComUnitCode = companyInfo[0],
                                           ComUnitName = companyInfo[1],
                                           EntryBy = Session["LoginName"].ToString(),
                                           EntryDate = DateTime.Now,
                                           ManufacId = Convert.ToInt32(null),
                                       };
...

   {
    ...
    protected void submitButton_Click(object sender, EventArgs e)
    {
        if (Validation())
        {
            ...
            List<CustomerPropUpdateDetailDAO> DtlList = new List<CustomerPropUpdateDetailDAO>();


            for (int i = 0; i < productGridView.Rows.Count; i++)
            {
               
                Label lblCustomerCode = (Label)productGridView.Rows[i].FindControl("lblCustomerCode");
                Label lblProviderTypeCode = (Label)productGridView.Rows[i].FindControl("lblProviderTypeCode");
                Label lblCustomerTypeCode = ((Label)productGridView.Rows[i].Cells[1].FindControl("lblCustomerTypeCode"));
                Label lblMarketCode = ((Label)productGridView.Rows[i].Cells[1].FindControl("lblMarketCode"));




                CustomerPropUpdateDetailDAO _DAO = new CustomerPropUpdateDetailDAO();

                _DAO.CustCode = string.IsNullOrEmpty(lblCustomerCode.Text.Trim()) ? null : lblCustomerCode.Text.Trim();
                _DAO.ProviderType = string.IsNullOrEmpty(lblProviderTypeCode.Text.Trim()) ? null : lblProviderTypeCode.Text.Trim();
                _DAO.CustTypeCode = string.IsNullOrEmpty(lblCustomerTypeCode.Text.Trim()) ? null : lblCustomerTypeCode.Text.Trim();
            ...
            CustomerPropUpdateMasterDAO aMaster = new CustomerPropUpdateMasterDAO();

       

            aMaster.TypeId =1;
            aMaster.ConvertType =rbType.SelectedValue;
         

            ResultInfo Res = _BonusCampaignNewDAL.SaveDoctorInfoForExcel(aMaster, DtlList,  Session["UserId"].ToString());
            if (Res.isSuccess == true)
            {


                ScriptManager.RegisterStartupScript(this, GetType(), "Popup", "successalert('" + "Operation successful!" + "','Success','DoctorImport.aspx');", true);
