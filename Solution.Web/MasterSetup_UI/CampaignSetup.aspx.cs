...
public partial class MasterSetup_UI_CampaignSetup : System.Web.UI.Page
{
    ...
    //private Label grouplabel, zonelabel, arealabel, teritorylabel, subTeritorylabel, marketlabel;
    protected void Page_Load(object sender, EventArgs e)
    {
        ...
        //grouplabel = (Label)IVMarketStructure.FindControl("groupLabel") as Label;
        //zonelabel = (Label)IVMarketStructure.FindControl("zoneLabel") as Label;
        //arealabel = (Label)IVMarketStructure.FindControl("regionLabel") as Label;
        //teritorylabel = (Label)IVMarketStructure.FindControl("areaLabel") as Label;
        //subTeritorylabel = (Label)IVMarketStructure.FindControl("territoryLabel") as Label;
        //marketlabel = (Label)IVMarketStructure.FindControl("marketLabel") as Label;
    ...
    private void HaideHierarchyInfo()
    {
        ZoneSelect.Visible = false;
        AreaSelect.Visible = false;
        TeritorySelect.Visible = false;
        SubTeritory.Visible = false;
        MarketSelect.Visible = false;

        //grouplabel.Visible = false;
        //zonelabel.Visible = false;
        //arealabel.Visible = false;
        //teritorylabel.Visible = false;
        //subTeritorylabel.Visible = false;
    ...
    private void GetOneRecord(string Id)
    {
        try
        {
            using (DataTable dt = _BonusCampaignNewDAL.GetCampaignSetupById(Id))
            {
                ...
                try
                {
                    using (DataTable aTable = _seedRepo.GetChemistTypeList())
                    {

                            ddlChemistType.DataSource = aTable;
                            ...
                            ListItem removeItem = ddlChemistType.Items.FindByValue("7");
                            ddlChemistType.Items.Remove(removeItem);
        ...
        try
        {
            using (DataTable dt = _seedRepo.GetChemistTypeList())
            {
                ...
                else
                {
                    ...
                    ddlChemistType.Items.Insert(0, new ListItem("Please Select From List", String.Empty));
                    ddlChemistType.SelectedIndex = 0;
                    ListItem removeItem = ddlChemistType.Items.FindByValue("6");
                    ddlChemistType.Items.Remove(removeItem);
                }
    ...
    public bool Validation()
    {
        ...
        {
            showMessageBox("please Add to List Market Structure !");

            return false;
        }


        //if (gv_Market.Rows.Count == 0 && hfCustomerId.Value=="")
        //{
    ...
    protected void btnSave_Click(object sender, EventArgs e)
    {
        ...
        if (Validation())
        {
                ...
                if (ddlChemistType.SelectedValue == "7")
                {
                    //CustomerTypeId
                  
                    DataTable aTable = _BonusCampaignNewDAL.GetActiveCustomerType();
                    if (aTable.Rows.Count > 0)
                    {
                        ...
                        {
                            aMaster.CustomerTypeId = Convert.ToInt32(aTable.Rows[i]["CustomerTypeId"].ToString());
                            aMaster.CampaignName = "";
                            aMaster.CampaignName = string.IsNullOrEmpty(txtCampaignName.Text.Trim()) ? null : txtCampaignName.Text.Trim() + " For " + aTable.Rows[i]["CustomerType"].ToString();
    ...
    public void AddCustomer()
    {
        ...
        for (int i = 0; i < gv_Customer.Rows.Count; i++)
        {
            ...
            TextBox lbl_CustomerName = ((TextBox)gv_Customer.Rows[i].Cells[1].FindControl("lbl_CustomerName"));
