...
public partial class PromoAlloc_PromoChallanReport : System.Web.UI.Page
{
    ...
    public void LoadDropDown()
    {
        ...
        aOrderInfoBll.LoadSC(ddlUnit, Session["UserId"].ToString());
    ...
    {
        string pram = "  AND M.IsForwarded = 1 AND M.ComUnitId IN (SELECT CompanyUnitId FROM tblUserCompanyUnit WHERE UserId = " + Session["UserId"].ToString() + ")";
        ...
        if (aTable.Rows.Count > 0)
        {
            ...
            for (int i = 0; i < masterGridView.Rows.Count; i++)
            {
                ...
                else
                {
                    ...
                    masterGridView.Rows[i].Cells[7].BackColor = col;
                    masterGridView.Rows[i].Cells[7].ForeColor = Color.White;
                    masterGridView.Rows[i].Cells[7].HorizontalAlign = HorizontalAlign.Center;
                }
            }
        }
        else
    ...
    protected void btnForward_Click(object sender, EventArgs e)
    {
        ...
        if (masterId != null)
        {
            if (aDal.ApproveChallanList(masterId))
            {
                ShowMessageBox("Received successfully !!!");
