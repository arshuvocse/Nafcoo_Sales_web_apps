...
public partial class PromoAlloc_PromoMIOTag : System.Web.UI.Page
{
    ...
    protected void groupname_SelectedIndexChanged(object sender, EventArgs e)
    {
        ...
        if (groupname.SelectedValue != "")
        {
            ...
            DataTable aTable = aTargetDal.GetPromoMioTagMasterId(groupname.SelectedValue);

            if (aTable.Rows.Count > 0)
            {
                id_mastetID.Value = (aTable.Rows[0].Field<Int32>("MIOTagId")).ToString(CultureInfo.InvariantCulture);
            }

            DataTable ds = aTargetDal.LoadMIO(groupname.SelectedValue);

            // Find Master id from list
            ...
            for (int i = 0; i < gv_List.Rows.Count; i++)
            {
                string isForwardAble = gv_List.DataKeys[i][0].ToString();
                CheckBox btnForward = (CheckBox)gv_List.Rows[i].Cells[0].FindControl("chkSelect");

                if (Convert.ToInt32(isForwardAble) != 0)
                {

                    if (Convert.ToInt32(groupname.SelectedValue) == Convert.ToInt32(isForwardAble))
                    {
