...
public partial class PromoAlloc_GroupWisePromoQtyEntry : System.Web.UI.Page
{
    ...
    private void LoadInitialInfo()
    {
        ...
        aTargetDal.LoadPromoWiseBrand(ddlBrand);
    ...
    protected void btnSave_Click(object sender, EventArgs e)
    {
        ...
        if (Validation())
        {
            ...
            for (int i = 0; i < gv_List.Rows.Count; i++)
            {
                ...
                string referenceId = gv_List.DataKeys[i][3].ToString();
                string gWPromoQtyId = gv_List.DataKeys[i][1].ToString();
                ...
                if (chkSelect.Checked)
                {
                    ...
                    _DAO.EmpInfoId = string.IsNullOrEmpty(hfEmpInfoId.Value)
                        ? (int?) null
                        : int.Parse(hfEmpInfoId.Value);

                    _DAO.TerritoryId = string.IsNullOrEmpty(hfTerritoryId.Value)
                        ? (int?) null
                        : int.Parse(hfTerritoryId.Value);
                    ...
                    _DAO.BrandId = groupname.SelectedIndex > 0 ? int.Parse(ddlBrand.SelectedValue) : (int?) null;
                    _DAO.PromoId = groupname.SelectedIndex > 0 ? int.Parse(ddlProduct.SelectedValue) : (int?) null;
                ...
                {
                    if (referenceId != "0" && gWPromoQtyId == "0")
                    {
                        aTargetDal.DeletePromoQty(Convert.ToInt32(referenceId));
                    }
                }
            ...
            {
                ScriptManager.RegisterStartupScript(this, GetType(), "Popup", "successalert('" + "Operation successful!" + "','Success','GroupWisePromoQtyEntry.aspx');", true);
    ...
    protected void ddlProduct_OnSelectedIndexChanged(object sender, EventArgs e)
    {
        ...
        if (groupname.SelectedValue != "")
        {
            DataTable aTable = aDal.GetPromoWiseQty(GenerateParameter(), groupname.SelectedValue);
            ...
            if (aTable.Rows.Count > 0)
            {
                ...
                gv_List.DataSource = aTable;
                gv_List.DataBind();

                for (int i = 0; i < gv_List.Rows.Count; i++)
                {
                    string isForwardAble = gv_List.DataKeys[i][1].ToString();
                    string qty = gv_List.DataKeys[i][2].ToString();
                    CheckBox btnForward = (CheckBox)gv_List.Rows[i].Cells[0].FindControl("chkSelect");
                    TextBox qtyTextBox = (TextBox)gv_List.Rows[i].Cells[5].FindControl("txtQty");

                    if (isForwardAble.Trim() != "0")
                    {
                        qtyTextBox.ReadOnly = true;
                        btnForward.Visible = false;
                        btnForward.ToolTip = "Already Exist In Challan !!";
                        System.Drawing.Color col = System.Drawing.ColorTranslator.FromHtml("#F2E7FE");
                        gv_List.Rows[i].BackColor = col;
                    }
                    else
                    {
                        if (qty != "")
                        {
                            btnForward.Checked = true;
                        }
                    }
        ...
        //DataTable dtdata = aDal.LoadCentralWearhouse();
    ...
    private String GenerateParameter()
    {
        ...
        {
            pram = pram + " AND Month = DateName( month , DateAdd( month , " + ddlmonth.SelectedValue  + " , 0 ) - 1 )";
        }

        if (ddlBrand.SelectedValue != "")
        {
            pram = pram + " AND BrandId = " + ddlBrand.SelectedValue;
        }
        

        if (ddlProduct.SelectedValue != "")
        {
            pram = pram + " AND promoId = " + ddlProduct.SelectedValue;
        }
        else
        {
            pram = pram + " AND promoId = 0";
        }
    ...
    protected void ddlBrand_OnSelectedIndexChanged(object sender, EventArgs e)
    {
        if (ddlBrand.SelectedValue != "")
        {
            aTargetDal.LoadPromoProductById(ddlProduct, Convert.ToInt32(ddlBrand.SelectedValue));
        ...
        ddlProduct_OnSelectedIndexChanged(null, null);
    }
