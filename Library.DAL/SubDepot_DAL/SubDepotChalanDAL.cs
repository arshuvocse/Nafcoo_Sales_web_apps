...
{
    public class SubDepotChalanDAL
    {
        private ClsCommonInternalDAL aCommonInternalDal = new ClsCommonInternalDAL();
        public bool SaveDataForChalanInfo(ChalanInfo aChalanInfo)
        {
            string insertQuery = @"insert into tblSubDepotChalanInfo (SChalanId,ChalanDate,ChalanNo,TrackNo,DriverName,FromComUnitCode,FromComUnitName,FromComUnitAddress,SubDepotCode,SubDepotName,SubDepotAddress,TotalValue,TotalVat,GrandTotal,ManufacId,IsDeliver) 
        ...
        {
            string insertQuery = @"insert into tblSubDepotChalanDetail (SChalanDetailsId,ProductCode,ProductName,Quantity,BatchNo,UnitPrice,Value,Vat,ValueWVat,SChalanId,DCStoreId) 
        ...
        {
            string query = @"UPDATE dbo.tblSubDepotChalanInfo SET IsDeliver='True' WHERE SChalanId='" + chalanId + "'";
      ...
      "     )";
        ...
        {
            string query = @"SELECT * FROM dbo.tblSubDepotChalanInfo
            LEFT JOIN dbo.tblSubDepot ON dbo.tblSubDepot.SubDepotCode=dbo.tblSubDepotChalanInfo.SubDepotCode
        ...
        {
            string query = @"SELECT tblSubDepotChalanDetail.DCStoreId,tblProduct.ProductName,*,tblSubDepotChalanInfo.ChalanDate AS ReceiveDate 
FROM dbo.tblSubDepotChalanInfo
            ...
            LEFT JOIN dbo.tblProduct ON dbo.tblSubDepotChalanDetail.ProductCode = dbo.tblProduct.ProductCode
            LEFT JOIN dbo.tblDCStore ON dbo.tblSubDepotChalanDetail.DCStoreId=dbo.tblDCStore.DCStoreId
            WHERE tblSubDepotChalanInfo.IsDeliver='False' and tblSubDepotChalanInfo.SChalanId='" + id + "'";
            return aCommonInternalDal.DataContainerDataTable(query, "SSIDB");
        }
...
{
    DataTable aDataTableEmpInfo = new DataTable();
    string query = @"SELECT format(ExpDate,'dd MMM, yyyy') as ExpDate, format(ReceiveDate,'dd MMM, yyyy') as ReceiveDate, ISNULL(VATAmountPerUnit,0)VATAmountPerUnit,ISNULL(VATPercentage,0)VATPercentage,UnitPrice,*,
            tblProduct.ProductCode AS PCode , tblProduct.ProductName AS PName 
            FROM dbo.tblDCStore
            LEFT JOIN dbo.tblProduct ON dbo.tblDCStore.ProductCode = dbo.tblProduct.ProductCode 
            LEFT JOIN dbo.tblCompanyUnit ON dbo.tblDCStore.ComUnitId = dbo.tblCompanyUnit.ComUnitId 
