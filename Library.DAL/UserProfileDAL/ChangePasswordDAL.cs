...
WHERE Emp.EmpInfoId="+Id;
        ...
        public bool UpdatePass(int UserId, string Pass)
        {
            ...
            try
            {
                ...
                if (UserId > 0)
                {
                    ...
                    string query =
                        @"update tblUser set Password=@Password,UpdateBy=@UserId,UpdateDate= GETDATE() where UserId=@UserId";
        ...
        public DataTable GetUserPassInfoDAL(string Id)
        {
              ...
              string queryStr = @"select u.LoginName,u.Password,emp.EmpName, uRR.RoleName, uType.UserType,dgs.DesigName from tblUser  u with (nolock)
