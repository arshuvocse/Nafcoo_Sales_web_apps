...
namespace Library.DAL.DoctorModule_DAL
{
    public class SetupDAL
    {
        ...
        public DataTable Get_PrescriptionList2(string param, string param2)
        {
            try
            {
                string queryStr = @"SELECT DISTINCT RGN.AreaCode +' : '+ RGN.AreaName AS Region, ARA.TerritoryCode +' : '+ ARA.TerritoryName AS Team, TTR.SubTerritoryCode +' : '+ TTR.SubTerritoryName AS Territory,
                                    EM.EmpMasterCode,EM.EmpName,ISNULL(TTP.TotalPrescription,0) TotalPrescription,ISNULL(TTD.TotalDoctor,0) TotalDoctor FROM tbl_PrescriptionMaster AS PM with (nolock)
                                    ...
                                    LEFT JOIN tblRegion AS CLS with (nolock) ON PM.RegionId = CLS.RegionId
                                    LEFT JOIN tblArea AS RGN with (nolock) ON PM.AreaId = RGN.AreaId
                                    LEFT JOIN tblTerritory AS ARA with (nolock)ON PM.TerritoryId = ARA.TerritoryId
                                    LEFT JOIN tblSubTerritory AS TTR with (nolock) ON PM.SubTerritoryId = TTR.SubTerritoryId
                                    ...
                                    LEFT JOIN (SELECT EntryBy,Count(PrescriptionId) TotalPrescription FROM tbl_PrescriptionMaster WHERE PrescriptionId IS NOT NULL " + param2 + " GROUP BY EntryBy) AS TTP ON PM.EntryBy = TTP.EntryBy LEFT JOIN (SELECT DISTINCT P.EntryBy,Count(CASE WHEN P.DoctorId IS NULL THEN rxOtherDoctor ELSE Dc.DoctorName END) TotalDoctor FROM tbl_PrescriptionMaster P LEFT JOIN tblDoctorMaster AS DC ON P.DoctorId = DC.DoctorId WHERE PrescriptionId IS NOT NULL " + param2 + " GROUP BY P.EntryBy) AS TTD ON PM.EntryBy = TTD.EntryBy WHERE PM.PrescriptionId IS NOT NULL " + param + " ORDER BY  EM.EmpMasterCode DESC";
        ...
        public DataTable Get_PrescriptionDetailList(string param)
        {
            try
            {
                accessManager.SqlConnectionOpen(DataBase.SalesDB);
                List<SqlParameter> aSqlParameters = new List<SqlParameter>();
                aSqlParameters.Add(new SqlParameter("@param", param));
                DataTable dt = accessManager.GetDataTable("sp_Get_PrescriptionDetailList", aSqlParameters);
                return dt;
            }
            catch (Exception e)
            {
