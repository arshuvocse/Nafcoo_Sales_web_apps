...
namespace Library.DAL.PromoAllocDAL
{
    public class GroupWisePromoQtyDAL
    {
        ...
        public void LoadPromoWiseBrand(DropDownList ddl)
        {
            ClsCommonInternalDAL aInternalDal = new ClsCommonInternalDAL();
            string queryStr = @"SELECT DISTINCT ProductBrandId,ProductSQName FROM tblBrandWisePromo AS BP
	                            LEFT JOIN tblProductSQ AS PSQ ON BP.BrandId = PSQ.ProductBrandId";
            aInternalDal.LoadDropDownValueWithoutDataBase(ddl, "ProductSQName", "ProductBrandId", queryStr);
        }
        public void LoadPromoProductById(DropDownList ddl,int brandId)
        {
            ClsCommonInternalDAL aInternalDal = new ClsCommonInternalDAL();
            string queryStr = @"SELECT DISTINCT PSQ.PromoProductId,PromoProductName FROM tblBrandWisePromo AS BP
	LEFT JOIN tblPromoProductName AS PSQ ON BP.PromoProductId = PSQ.PromoProductId WHERE BrandId = " + brandId;
        ...
        public ResultInfo SaveDetail(List<GroupWisePromoQtyDAO> _MarketDtls, string sessionUser)
        {
            ...
            try
            {
                accessManager.SqlConnectionOpen(DataBase.SalesDB);
                    ...
                    aSQL.Add(new SqlParameter("@BrandId", item.BrandId ?? (object)DBNull.Value));
                    aSQL.Add(new SqlParameter("@PromoId", item.PromoId ?? (object)DBNull.Value));
...
LEFT JOIN dbo.tblProduct pro  with (nolock) ON mas.ProductId=pro.ProductId";
        ...
        public DataTable GetPromoWiseQty(string pram, string promoGroupId)
        {
            string query = @"SELECT ISNULL(PM.RefId,0) RefId,ISNULL(GWPromoQtyId,0) GWPromoQtyId,CASE WHEN TransactionQTY IS NOT NULL THEN CONVERT(VARCHAR,TransactionQTY) ELSE '' END Qty,tblMBEInfo.SubTerritoryId TerritoryId , EmpMasterCode,EmpName,SubTerritoryCode+' : '+ SubTerritoryName TerritoryName,tblEmpGeneralInfo.EmpInfoId,
...
LEFT JOIN tblSubTerritory ON tblSubTerritory.SubTerritoryId = tblMBEInfo.SubTerritoryId
LEFT JOIN (SELECT MIOId,TransactionQTY,D.GWPromoQtyId,PW.GWPromoQtyId AS RefId FROM tblGroupWisePromoQty AS PW 
LEFT JOIN (SELECT GWPromoQtyId FROM tbl_Promo_ChallanDetails) AS D ON PW.GWPromoQtyId = D.GWPromoQtyId WHERE MIOId IS NOT NULL " + pram + ") AS PM ON tblPromoMIOTagDetail.MIOId = PM.MIOId WHERE tblMBEInfo.IsActive = 1 AND PromoGroupId=' " + promoGroupId + "' ORDER BY EmpMasterCode asc ";
            return aCommonInternalDal.DataContainerDataTable(query, "SSIDB");
        }

        public bool DeletePromoQty(int gwPromoQtyId)
        {
            string insertQuery = @"DELETE FROM tblGroupWisePromoQty WHERE GWPromoQtyId = '" + gwPromoQtyId + "'  ";
