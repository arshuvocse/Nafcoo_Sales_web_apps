...
namespace Library.DAL.SInventory_DAL
{
    public class SCtoWHTransferDal
    {
        ...
        {
            string insertQuery = @"insert into tblDepotToWHChalanInfo (SChalanId,ChalanDate,ChalanNo,TrackNo,DriverName,FromComUnitCode,FromComUnitName,FromComUnitAddress,WHCode,WHName,WHAddress,ForwardBy,ForwardDate,TotalValue,TotalVat,GrandTotal,ManufacId,IsDeliver) 
            values (" + aChalanInfo.ChalanId + ",'" + aChalanInfo.ChalanDate + "','" + aChalanInfo.ChalanNo + "','" + aChalanInfo.TrackNo + "','" + aChalanInfo.DriverName + "','" + aChalanInfo.FromComUnitCode + "','" + aChalanInfo.FromComUnitName + "','" + aChalanInfo.FromComUnitAddress + "','" + aChalanInfo.WHCode + "','" + aChalanInfo.WHName + "','" + aChalanInfo.WHAddress + "','" + aChalanInfo.ForwardBy + "','" + aChalanInfo.ForwardDate + "','" + aChalanInfo.TotalValue + "','" + aChalanInfo.TotalVat + "','" + aChalanInfo.GrandTotal + "','" + aChalanInfo.ManufacId + "','" + false + "')";
        ...
        public DataTable GetDepotToWhChalan(string challanId)
        {
            DataTable aDataTable = new DataTable();

            string query = @"SELECT FromComUnitName,ChalanNo,ChalanDate,TrackNo,DriverName,ProductCode,ProductName,Quantity,BatchNo,UnitPrice,Value,Vat,ValueWVat,P.Purpose FROM tblDepotToWHChalanInfo AS M
							 LEFT JOIN tblDepotToWHChalanDetail AS D ON M.SChalanId = D.SChalanId
							 LEFT JOIN tblPurpose AS P ON D.PurposeId = P.PurposeId";
        ...
        public DataTable GetB2bTransferInfo(string getPrameterList)
        {
            string query = @"SELECT ISNULL(FUE.EmpMasterCode,'') EmpMasterCode, ISNULL(FUE.EmpName,FU.UserName) AS ForwardBy, FORMAT(ForwardDate, 'dd-MMM-yyyy hh:mm:ss tt') ForwardDate, ISNULL(RVUE.EmpName,RVU.UserName) AS ReceiveByBy,
                             ISNULL(RVUE.EmpMasterCode,'') ReceiveEmpMasterCode,FORMAT(M.ReceiveDate, 'dd-MMM-yyyy hh:mm:ss tt')  AS ChalanReceiveDate, 
                             M.SChalanId,M.ChalanDate,M.ChalanNo,TrackNo,DriverName,FromComUnitCode,FromComUnitName,D.ProductCode,D.ProductName,StockUOMName,D.BatchNo,D.Quantity FROM tblDepotToWHChalanInfo AS M
                             LEFT JOIN tblDepotToWHChalanDetail AS D ON M.SChalanId = D.SChalanId
                             LEFT JOIN tblProduct AS PD ON D.ProductCode = PD.productCode
                             LEFT JOIN tblStockUOM AS UM ON PD.StockUOMId = UM.StockUOMId
                             
                             LEFT JOIN tblUser AS FU ON M.ForwardBy = FU.UserId
                             LEFT JOIN tblEmpGeneralInfo AS FUE ON FU.EmpInfoId = FUE.EmpInfoId
                             LEFT JOIN tblUser AS RVU ON M.ReceiveBy = RVU.UserId
                             LEFT JOIN tblEmpGeneralInfo AS RVUE ON RVU.EmpInfoId = RVUE.EmpInfoId	
                             
                             WHERE IsTransfer = 'OK'" + getPrameterList;
