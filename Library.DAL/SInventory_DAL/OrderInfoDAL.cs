...
'0'BonusQty,tblInvoiceDetail.TotalQuantity AS TotalQty,ISNULL(tblReturnInvoiceDetail.TotalQuantity,0) AS TQty FROM dbo.tblInvoice
        ...
        public Int32 GenerateInvoiceByOrderId(int orderId, int userId, string batchno)
        {
            List<SqlParameter> aSqlParameterList = new List<SqlParameter>();
            aSqlParameterList.Add(new SqlParameter("@OrderId", orderId));
            aSqlParameterList.Add(new SqlParameter("@UserId", userId));
...
//            string query = @"SELECT tblOrder.CustomerType,tblOrder.MarketName_Ord MarketName,*,
//            CASE WHEN ISNULL(LMT.LimitAmount,0) = 0  AND CSTMR.TermOfPayment != 'Credit' THEN 'Yes' ELSE CASE WHEN (ISNULL(LMT.LimitAmount,0) = ISNULL(DD.Due,0) OR ISNULL(LMT.LimitAmount,0) > ISNULL(DD.Due,0)) THEN 'Yes' ELSE 'Yes' END END IsInvoiceAble FROM dbo.tblOrder  With (NOLOCK) 
//            LEFT JOIN (SELECT CustomerMasterId,LimitAmount FROM tblCustomerCreditLimit WHERE ActionStatus = 'Accepted' AND IsActive = 1) AS LMT ON tblOrder.CustomerMasterId = LMT.CustomerMasterId
//            LEFT JOIN (SELECT CustomerMasterId,SUM(ISNULL(DeliveryTpGrandTotal,0))- SUM(ISNULL(PaymentAmount,0)) Due FROM tblInvoice AS INV 
//            WHERE INV.DeliveryInvoiceStatus IN ('Partial','Full') GROUP BY CustomerMasterId) AS DD ON tblOrder.CustomerMasterId = DD.CustomerMasterId 
//            LEFT JOIN tblCustMaster CSTMR ON tblOrder.CustomerMasterId = CSTMR.CustomerMasterId
//            WHERE IsInvoice = 0 and OrderType='Regular' and   tblOrder.ComUnitId='" + comunitId + "' AND tblOrder.DistributionRouteId='" + manufacId + "'  AND IsInvoice =0  and  tblOrder.ActionStatus='2' and  ISNULL(tblOrder.IsSubDepo,0) = 0";

            string query = @"SELECT ComUnitId,ManufacId,OrderId,0 IsInvoiceAble,OrderCode,SubmissionDate,EntryDate,CustomerCode,CustomerName,tblOrder.MarketName_Ord AS MarketName,
                             GrossValue,CustomerType,DeliveryDate,Remarks,CustomerCode,CustomerMasterId FROM dbo.tblOrder  With (NOLOCK) 
                             WHERE IsInvoice = 0 and OrderType='Regular' and   tblOrder.ComUnitId='" + comunitId + "' AND tblOrder.DistributionRouteId='" + manufacId + "'  AND IsInvoice =0  and  tblOrder.ActionStatus='2' ";
        ...
        {
            string query = @"SELECT tblInvoice.CustomerMasterId,tblMarket.MarketId,* FROM dbo.tblOrder     
            inner JOIN dbo.tblInvoice ON dbo.tblOrder.OrderId=dbo.tblInvoice.OrderId 
            inner JOIN dbo. View_CustomerMaster V ON dbo.tblOrder.CustomerCode = V.CustomerCode
            inner JOIN dbo.tblCompanyUnit ON dbo.tblOrder.ComUnitCode=dbo.tblCompanyUnit.ComUnitCode
            inner JOIN dbo.tblMarket ON V.MarketCode = dbo.tblMarket.MarketCode  
...
//            return aCommonInternalDal.DataContainerDataTable(query, "SSIDB");
        ...
        {
//            string query = @"            
//            SELECT 'True' AS chkStatus ,tblInvoice.CustomerMasterId,tblMarket.MarketId,* FROM dbo.tblOrder With (nolock)
//            LEFT JOIN dbo.tblInvoice  With (nolock) ON dbo.tblOrder.OrderId=dbo.tblInvoice.OrderId 
//            LEFT JOIN dbo. tblCustMaster  V   With (nolock)  ON dbo.tblOrder.CustomerCode = V.CustomerCode
//            LEFT JOIN dbo.tblCompanyUnit  With (nolock) ON dbo.tblOrder.ComUnitCode=dbo.tblCompanyUnit.ComUnitCode
//            LEFT JOIN dbo.tblMarket  With (nolock) ON tblOrder.MarketId = dbo.tblMarket.MarketId  
//            WHERE  TpGrandTotal > 0 " + param;


            string query = @"SELECT tblInvoice.ComUnitId,ManufacId,tblOrder.OrderId,InvoiceId,MarketId,tblOrder.CustomerMasterId,'True' AS chkStatus,IsAdjustInvoice,InvoiceNo, tblInvoice.CustomerMasterId,InvoiceDate,CustomerCode,
			 CustomerName,MarketName,SubmissionDate,TpGrandTotal FROM dbo.tblOrder With (nolock)
            LEFT JOIN dbo.tblInvoice  With (nolock) ON dbo.tblOrder.OrderId=dbo.tblInvoice.OrderId 
            LEFT JOIN dbo.tblCompanyUnit  With (nolock) ON dbo.tblOrder.ComUnitCode=dbo.tblCompanyUnit.ComUnitCode
            WHERE  TpGrandTotal > 0  " + param;
...
//where mas.DCId="+ comunitId;
        ...
        {
            string query = @"SELECT IsCampaignProduct,tblOrderDetail.IsSpDis,tblOrderDetail.CampaignName,* FROM dbo.tblOrder
            LEFT JOIN dbo.tblCustMaster ON dbo.tblOrder.CustomerCode = dbo.tblCustMaster.CustomerCode
            LEFT JOIN dbo.tblCompanyUnit ON dbo.tblOrder.ComUnitCode=dbo.tblCompanyUnit.ComUnitCode
        ...
        public DataTable GetTradeTerm(string amount)
        {
            //string query = @"SELECT * FROM dbo.tblTradePolicyNew WHERE '" + amount + "' BETWEEN MinAmount AND MaxAmount";
            string query = @"SELECT * FROM tbl_BonusCampaignNewMaster AS BM LEFT JOIN tbl_BonusCampaignNewDetail AS BMD ON BM.CampgainMasterId = BMD.CampaignMasterId WHERE '" + amount + "' BETWEEN MinAmount AND BM.MaxAmount AND GETDATE() BETWEEN FromDate AND Todate";
            return aCommonInternalDal.DataContainerDataTable(query, "SSIDB");
        }
...
//where IsInvoice=0 and ComUnitId=" + comunitId;
            ...
            where IsInvoice=0 and  tblOrder.ActionStatus='2'  and ComUnitId=" + dis + "  order by tblRouteInformationMaster.RouteName asc";
...
where ISNULL(TpGrandTotal,0) > 0 AND DelivaryInvoiceNo  is null  and Inv.ComUnitId=" + dis;
            aInternalDal.LoadDropDownValueWithoutDataBase(ddl, "DistributionRouteName", "DistributionRouteId", queryStr);
        }
        ...
        {
            string query = @"SELECT tblOrderDetail.ISGiftProduct,(case when  tblOrderDetail.DiscountAmount>0 then 0  else 1 END)IsCampaignProduct,*,tblInvoiceDetail.UnitVatAmount AS UnitVAT,'0'SL,'0'StockQty,UnitPrice,tblInvoiceDetail.UnitVatAmount,TotalQuantity AS Quantity,tblOrderDetail.TradePrice TotalPrice,TotalPriceVatAmount AS VAT,DiscountPercentage,tblInvoiceDetail.DiscountAmount,tblInvoiceDetail.NetAmount AS NetPrice,'0'BonusQty,TotalQuantity AS TotalQty FROM dbo.tblInvoice
            LEFT JOIN dbo.tblInvoiceDetail ON dbo.tblInvoice.InvoiceId = dbo.tblInvoiceDetail.InvoiceId
        ...
        {
            string query = @"SELECT DiscountPercent,DiscountAmount,tblOrderDetail.CampaignName,* FROM dbo.tblOrder
            LEFT JOIN dbo.tblCustMaster ON dbo.tblOrder.CustomerCode = dbo.tblCustMaster.CustomerCode
            LEFT JOIN dbo.tblCompanyUnit ON dbo.tblOrder.ComUnitCode=dbo.tblCompanyUnit.ComUnitCode
            LEFT JOIN dbo.tblMarket ON dbo.tblCustMaster.MarketId = dbo.tblMarket.MarketId
            LEFT JOIN dbo.tblOrderDetail ON dbo.tblOrder.OrderId=dbo.tblOrderDetail.OrderId
