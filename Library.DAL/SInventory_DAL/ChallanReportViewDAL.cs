...
";
       ...
       {
           string query = @"UPDATE tblRequisition SET SubmitDate='" + aCompanyUnit.IssuChalanDate + "', PickingDate='" + aCompanyUnit.IssuChalanDate + "', IssuChalanDate='" + aCompanyUnit.IssuChalanDate + "',DriverName='" + aCompanyUnit.DriverName + "',TruckNo='" + aCompanyUnit.TruckNo + "'  WHERE ReqId='" + aCompanyUnit.ReqId + "'";
           return aCommonInternalDal.UpdateDataByUpdateCommand(query, "SSIDB");
       }

       public Requesition SalesCenterEditLoad(string ComUnitId)
       {
           string query = @"SELECT mas.IssuChalanDate, mas.DriverName, mas.TruckNo, * FROM dbo.tblRequisition mas where mas.ReqId= '" + ComUnitId + "'";
           ...
           if (dataReader != null)
           {
               ...
               {
                   aCompanyUnit.ReqId = Int32.Parse(dataReader["ReqId"].ToString());
                   aCompanyUnit.DriverName = dataReader["DriverName"].ToString();
                   aCompanyUnit.TruckNo = dataReader["TruckNo"].ToString();
                   aCompanyUnit.IssuChalanDate = Convert.ToDateTime(dataReader["IssuChalanDate"]);
 ...
 WHERE ReqId ="+id;
    ...
    "    ( StockInTransfarId , "+
     ...
     "     )";
       ...
       {
           string query = @"UPDATE dbo.tblRequisition SET ReceiveIssue='OK',ReceiveIssueDate='" + rcvDate + "' WHERE ReqId='" + reqId.Trim() + "'";
           return aCommonInternalDal.UpdateDataByUpdateCommand(query, "SSIDB");
       }
...
LEFT JOIN dbo.tblWHStockInDetail M ON W.MigoDetailID = M.WHStockInDetailID
INNER JOIN dbo.tblUnitPrice UP ON T.ProductCode = UP.ProductCode
WHERE ComUnitId='" + unit + "' and Submit='OK' AND SubmitDate BETWEEN '" + date + "' and '" + todate + "' group by P.ProductCode,P.ProductName ";
           return aCommonInternalDal.DataContainerDataTable(query, "SSIDB");
       }
