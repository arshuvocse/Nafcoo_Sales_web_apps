...
namespace Library.DAL.SInventory_DAL
{
    public class DCStockReportDAL
    {
        ...
        {
            string query = @"select * from tblCompanyUnit where ComUnitId='"+comUnitId.Trim()+"'";
  ...
  SELECT p.ProductCode,p.ProductName,DCS.PackSize,DCS.
        ...
        public DataTable DCReportDetailDataDALDtls(string comUnitId)
        {
            string query = "";
            if (comUnitId != "")
            {
...
SELECT  com.ComUnitCode ComUnitCode,  com.ComUnitCode+' : '+com.ComUnitName ComUnitName, p.ProductCode,p.ProductName,PKS.PackSizeName PackSize,SUM(DCS.StockQty) AS AQty,BatchNo,MfgDate MfgDate, ExpDate
                  ,0 BookFDel, 0 Tqty, 0 RQty, 0 BQty FROM dbo.tblDCStore DCS 
                INNER JOIN dbo.tblProduct p ON DCS.ProductCode = p.ProductCode
				LEFT JOIN tblPackSize AS PKS ON P.PackSizeId = PKS.PackSizeId
                --INNER JOIN tblStockInTransfar ST ON ST.StockInTransfarId = DCS.StockInTransfarId

					LEFT JOIN tblCompanyUnit AS com ON com.ComUnitId = DCS.ComUnitId
                --INNER JOIN dbo.tblCentralStore CS ON CS.ReceiveId = ST.ReceiveId
                --INNER JOIN dbo.tblWHStockInDetail MD ON MD.WHStockInDetailID = CS.MigoDetailID
                WHERE DCS.StockQty>0 AND DCS.StockCondition='Available' and com.ComUnitId='" + comUnitId + @"'

  	GROUP BY com.ComUnitCode, com.ComUnitCode+' : '+com.ComUnitName, p.ProductCode,p.ProductName,PKS.PackSizeName ,BatchNo,MfgDate,ExpDate
...
INNER JOIN dbo.tblWHStockInDetail MD ON MD.WHStockInDetailID = CS.MigoDetailID
 ...
 where vTblTotal.ProductCode='" + Pcode + "'GROUP BY vTblTotal.ProductCode ,vTblTotal.ProductName ,vTblTotal.PackSize ,vTblTotal.BatchNo ,vTblTotal.MfgDate ,vTblTotal.ExpDate ";
            }
            return aCommonInternalDal.DataContainerDataTable(query, "SSIDB");
        }
        ...
        public DataTable SubDeportStockReportDetailDataDAL(string comUnitId)
        {
            ...
            if (comUnitId != "")
            {
                ...
                where vTblTotal.SubDepotId='" + comUnitId + "' GROUP BY vTblTotal.SubDepotId,Cu.SubDepotCode,Cu.SubDepotName,vTblTotal.ProductCode, vTblTotal.ProductName ,vTblTotal.PackSize ,vTblTotal.BatchNo ,vTblTotal.MfgDate ,vTblTotal.ExpDate"; 
