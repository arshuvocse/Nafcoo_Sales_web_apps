...
namespace Library.DAL.SInventory_DAL
{
    public class InvoiceDAL
    {
        ...
        public DataTable InvoiceModification(string pram)
        {
            try
            {
                accessManager.SqlConnectionOpen(DataBase.SalesDB);
                List<SqlParameter> aSqlParameters = new List<SqlParameter>();
                aSqlParameters.Add(new SqlParameter("@InvoiceNo", pram));

                DataTable dt = accessManager.GetDataTable("sp_GET_InvoiceModification", aSqlParameters);
                return dt;
            }
            catch (Exception e)
            {
        ...
        public DataTable InvoiceNo( )
        {
            try
            {
                accessManager.SqlConnectionOpen(DataBase.SalesDB);
                List<SqlParameter> aSqlParameters = new List<SqlParameter>();
                DataTable dt = accessManager.GetDataTable("sp_GET_InvoiceNo_DDL");
                return dt;
            }
            catch (Exception e)
            {
        ...
        public void InvoiceNumber(DropDownList aDropDownList)
        {
            string query = @" Select InvoiceNo, InvoiceId from tblInvoice ";
            aCommonInternalDal.LoadDropDownValue(aDropDownList, "InvoiceNo", "InvoiceId", query, "SSIDB");
        }
        ...
        public DataTable get_pay_Slip_DAL(string invNo)
        {
            string query = @"Select CustPayDetailId , PayImg from tblCustPayDetail Where  CustPayDetailId=" + invNo;
     ...
     "     ReturnInvoiceNo , " +
      ...
      "   ) ";
        ...
        public bool SaveDataForInvoiceDetails(InvoiceDetail aInvoiceDetail)
        {
            ...
            aSqlParameterList.Add(new SqlParameter("@ProductCode", (object)aInvoiceDetail.ProductCode ?? DBNull.Value));
            aSqlParameterList.Add(new SqlParameter("@ProductName", (object)aInvoiceDetail.ProductName ?? DBNull.Value));
            aSqlParameterList.Add(new SqlParameter("@PackSize", (object)aInvoiceDetail.PackSize ?? DBNull.Value));
            aSqlParameterList.Add(new SqlParameter("@BatchNo", (object)aInvoiceDetail.BatchNo ?? DBNull.Value));
            aSqlParameterList.Add(new SqlParameter("@ReceiveDate", (object)aInvoiceDetail.ReceiveDate ?? DBNull.Value));
            aSqlParameterList.Add(new SqlParameter("@ExpDate", (object)aInvoiceDetail.ExpDate ?? DBNull.Value));
            aSqlParameterList.Add(new SqlParameter("@CostPrice", (object)aInvoiceDetail.CostPrice ?? DBNull.Value));
            aSqlParameterList.Add(new SqlParameter("@UnitPrice", (object)aInvoiceDetail.UnitPrice ?? DBNull.Value));
            aSqlParameterList.Add(new SqlParameter("@UnitVatAmount", (object)aInvoiceDetail.UnitVatAmount ?? DBNull.Value));
            aSqlParameterList.Add(new SqlParameter("@Quantity", (object)aInvoiceDetail.Quantity ?? DBNull.Value));
            aSqlParameterList.Add(new SqlParameter("@BonusQuantity", (object)aInvoiceDetail.BonusQuantity ?? DBNull.Value));
            aSqlParameterList.Add(new SqlParameter("@TotalQuantity", (object)aInvoiceDetail.TotalQuantity ?? DBNull.Value));
            aSqlParameterList.Add(new SqlParameter("@TotalPrice", (object)aInvoiceDetail.TotalPrice ?? DBNull.Value));
            aSqlParameterList.Add(new SqlParameter("@TotalPriceVatAmount", (object)aInvoiceDetail.TotalPriceVatAmount ?? DBNull.Value));
            aSqlParameterList.Add(new SqlParameter("@DiscountPercentage", (object)aInvoiceDetail.DiscountPercentage ?? DBNull.Value));
            aSqlParameterList.Add(new SqlParameter("@DiscountAmount", (object)aInvoiceDetail.DiscountAmount ?? DBNull.Value));
            aSqlParameterList.Add(new SqlParameter("@NetAmount", (object)aInvoiceDetail.NetAmount ?? DBNull.Value));
            aSqlParameterList.Add(new SqlParameter("@InvoiceId", (object)aInvoiceDetail.InvoiceId ?? DBNull.Value));
            aSqlParameterList.Add(new SqlParameter("@DCStoreId", (object)aInvoiceDetail.DCStoreId ?? DBNull.Value));
            aSqlParameterList.Add(new SqlParameter("@OrderDetailsId", (object)aInvoiceDetail.OrderDetailsId ?? DBNull.Value));
            aSqlParameterList.Add(new SqlParameter("@Campaign", (object)"N")); // Fixed value, no null check needed
            aSqlParameterList.Add(new SqlParameter("@ISGiftProduct", (object)aInvoiceDetail.ISGiftProductforInv ?? DBNull.Value));
            aSqlParameterList.Add(new SqlParameter("@IsCampaignProduct", (object)aInvoiceDetail.IsCampaignProductforInv ?? DBNull.Value));
            aSqlParameterList.Add(new SqlParameter("@SpecialAmount", (object)aInvoiceDetail.SpecialAmount ?? DBNull.Value));
            aSqlParameterList.Add(new SqlParameter("@AdjustmentAmount", (object)aInvoiceDetail.AdjustmentAmount ?? DBNull.Value));


            if (aCommonInternalDal.RunStoreProcedure("sp_I_InvoiceDetail", aSqlParameterList, "SSIDB") > 0)
            {
                status = true;
            }
 ...
 "  VALUES  ( '" + aInvoiceDetail.InvoiceDetailId + "' , " +
      ...
      "     )";
        ...
        public DataTable ProductInfoDAL(string comUnitId, string productCode)
        {
            ...
            " ISNULL(UP.VATPercentage,0)VATPercentage,P.ProductId  FROM " +
            " dbo.tblProduct P  " +
            " LEFT JOIN dbo.tblUnitPrice UP ON P.ProductCode = UP.ProductCode  " +
            " LEFT JOIN (select ComUnitId,ProductCode,CASE WHEN TotalQty IS NULL OR TotalQty < 0 THEN 0 ELSE TotalQty END AS TotalQty from View_DCStoreCurrentStock WHERE ComUnitId='" + comUnitId.Trim() + "' AND ProductCode='" + productCode.Trim() + "') VCS  " +
            " ON P.ProductCode=VCS.ProductCode   where P.ProductCode='" + productCode.Trim() + "' ";
            return aCommonInternalDal.DataContainerDataTable(query, "SSIDB");
        }
        public DataTable ProductInfoIdDAL(string comUnitId, string productCode)
        {
            ...
            string query = @"   SELECT P.ProductCode,(P.ProductName+':'+P.PackSize) as  ProductName,P.PackSize, " +
                           ...
                           " dbo.tblProduct P  " +
                           " LEFT JOIN dbo.tblUnitPrice UP ON P.ProductCode = UP.ProductCode  " +
                           " LEFT JOIN (select ComUnitId,VDC.ProductCode, TotalQty from View_DCStoreCurrentStock VDC left join tblProduct P on VDC.ProductCode = P.ProductCode WHERE ComUnitId='" + comUnitId.Trim() + "' AND ProductId='" + productCode.Trim() + "') VCS  " +
                           " ON P.ProductCode=VCS.ProductCode   where P.ProductId='" + productCode.Trim() + "' ";
            return aCommonInternalDal.DataContainerDataTable(query, "SSIDB");
        }
...
where I.ComUnitId= '" + Dcid + "' and tblD.ManufacId='" + ManufId + "' and I.AreaCode='" + tr + "' and InvoiceDate='" + invDate + "' order by OrderNo";
        ...
        {
            string query = @" SELECT DISTINCT '" + Code + @"'TopSheetGenCode,  ord.OrderSenderName as ReturnInvoiceId,IV.AdjustAmount,IV.ReceivableAmount,IV.FixedCustomer ,IV.DeliveryPersonName, CM.CellNo as DeliveryPersonPhNo,IV.InvoiceId,IV.InvoiceNo,IV.CreateDate InvoiceDate,IV.OrderNo,ord.EntryDate OrderDate,ct.CustomerType as TpTotal,IV.TpVat,(IV.TpDiscount+isnull(IV.TotalSpecialAmount,0))TpDiscount,IV.TpGrandTotal,CU.ComUnitCode,REPLACE(CU.ComUnitName, ' Distribution Center', '')  as CompanyName,  (CU.Address) AS CUAddress,  CM.CustomerCode,CM.CustomerName, ISNULL(CM.Address,'') + ','+  ISNULL(ThanaName,'') + ',' + ISNULL(DST.DistrictName,'') AS CMAddress,CM.CellNo AS Address2,CT.CustomerType as CategoryName,PT.PaymentTypeName,  emp.EmpMasterCode MiaCode,emp.EmpName MiaName,RM.RouteName as UserName, 0 AS AdjustmentValue,EGI.EmpName + '( ' + EGI.EmpMasterCode + ')' AS EntryBy
   FROM tblInvoice IV
    ...
    LEFT JOIN tblPaymentType PT ON IV.PaymentTypeId = PT.PaymentTypeId
	   ...
	   LEFT JOIN View_CustomerAndFieldForces AS V ON CM.CustomerCode = V.CustomerCode
       LEFT JOIN tblMarket AS MKT ON CM.MarketId = MKT.MarketId
       LEFT JOIN tbl_Thana AS THN ON MKT.ThanaId = THN.ThanaId
       LEFT JOIN tbl_District AS DST ON THN.district_id = DST.DistrictId
	   LEFT JOIN tblRouteInformationMaster AS RM ON V.RouteInformationMasterId = RM.RouteInformationmasterId
        LEFT JOIN tblUser U ON IV.UserId = U.UserId
		LEFT JOIN tblEmpGeneralInfo AS EGI ON U.EmpInfoId = EGI.EmpInfoId
         " +
                //" WHERE IV.InvoiceNo='" + invNo.Trim() + "'";
...
//        public DataTable InvoiceMainDataForReport(string invNo, string Code)
//        {
//            string query = @" SELECT '" + Code + @"'TopSheetGenCode,  ord.OrderSenderName as ReturnInvoiceId,IV.AdjustAmount,IV.ReceivableAmount,IV.FixedCustomer ,IV.DeliveryPersonName,IV.DeliveryPersonPhNo,IV.InvoiceId,IV.InvoiceNo,IV.InvoiceDate,IV.OrderNo,IV.OrderDate,ct.CustomerType as TpTotal,IV.TpVat,(IV.TpDiscount+isnull(IV.TotalSpecialAmount,0))TpDiscount,IV.TpGrandTotal,CU.ComUnitCode,CU.ComUnitName as CompanyName,  (CU.Address) AS CUAddress,  CM.CustomerCode,CM.CustomerName, (CM.Address) AS CMAddress,CM.Addrees2,ptt.ProgramTypeName as CategoryName,PT.PaymentTypeName,  emp.EmpMasterCode MiaCode,emp.EmpName MiaName,CM.MarketName as UserName
//   FROM tblInvoice IV
//
//    INNER JOIN dbo.tblOrder ord ON IV.OrderId = ord.OrderId
//    LEFT JOIN tblProgramType ptt ON ord.ProgramTypeId = ptt.ProgramTypeId
...
//        }
        ...
        {
            string query = @"SELECT tblOrder.OrderSenderName as ReturnInvoiceId,IV.AdjustAmount,IV.ReceivableAmount,IV.FixedCustomer ,IV.DeliveryPersonName,IV.DeliveryPersonPhNo,IV.InvoiceId,IV.InvoiceNo,IV.InvoiceDate,IV.OrderNo,IV.OrderDate,IV.CustomerType as TpTotal,IV.TpVat,(IV.TpDiscount+isnull(IV.TotalSpecialAmount,0))TpDiscount,IV.TpGrandTotal,CU.ComUnitCode,CU.ComUnitName as CompanyName,  (CU.Address) AS CUAddress,  CM.CustomerCode,CM.CustomerName, (CM.Address) AS CMAddress,CM.Addrees2,IV.Types as CategoryName,PT.PaymentTypeName,  MIA.EmpMasterCode MiaCode,MIA.EmpName MiaName,tblOrder.MarketName_Ord as UserName  FROM tblInvoice IV 
 ...
 LEFT JOIN tblCompanyUnit CU ON IV.ComUnitId = CU.ComUnitId 
  LEFT JOIN tblCustMaster CM ON IV.CustomerMasterId=CM.CustomerMasterId 
   LEFT JOIN tblPaymentType PT ON IV.PaymentTypeId=PT.PaymentTypeId 
    LEFT JOIN tblUser U ON IV.UserId=U.UserId  
	LEFT JOIN dbo.tblCustCategory CC ON CM.CategoryId=CC.CategoryId  
	 LEFT JOIN tblOrder on tblOrder.OrderId =IV.OrderId 
    LEFT JOIN dbo.tblEmpGeneralInfo MIA ON tblOrder.MIOId=MIA.EmpInfoId  " +
                //" WHERE IV.InvoiceNo='" + invNo.Trim() + "'";
        ...
        {
            string query = @"SELECT tblOrder.OrderSenderName as ReturnInvoiceId,IV.AdjustAmount,IV.ReceivableAmount,IV.FixedCustomer ,IV.DeliveryPersonName,IV.DeliveryPersonPhNo,IV.InvoiceId,IV.InvoiceNo,IV.InvoiceDate,IV.OrderNo,IV.OrderDate,IV.CustomerType as TpTotal,IV.TpVat,(IV.TpDiscount+isnull(IV.TotalSpecialAmount,0))TpDiscount,IV.TpGrandTotal,CU.ComUnitCode,CU.ComUnitName as CompanyName,  (CU.Address) AS CUAddress,  CM.CustomerCode,CM.CustomerName, (CM.Address) AS CMAddress,CM.Addrees2,IV.Types as CategoryName,PT.PaymentTypeName,  MIA.EmpMasterCode MiaCode,MIA.EmpName MiaName,tblOrder.MarketName_Ord as UserName  FROM tblInvoice IV   with (nolock)
 ...
 LEFT JOIN tblCompanyUnit CU   with (nolock) ON IV.ComUnitId = CU.ComUnitId 
  LEFT JOIN tblCustMaster CM  with (nolock) ON IV.CustomerMasterId=CM.CustomerMasterId 
   LEFT JOIN tblPaymentType PT  with (nolock) ON IV.PaymentTypeId=PT.PaymentTypeId 
	...
	LEFT JOIN dbo.tblCustCategory CC  with (nolock) ON CM.CategoryId=CC.CategoryId  
   ...
   INNER JOIN dbo.tblInvoice I ON I.InvoiceId = IV.InvoiceId  
     ...
     left JOIN dbo.tblProductDiscount PD ON PD.CustomerMasterId = I.CustomerMasterId AND PD.ProductCode = IVD.ProductCode   WHERE IVD.InvoiceId in (" + invNo.Trim() + ")  order by IVD.ProductName asc";
        ...
        public bool DCStockInDAL(DCStockNew aDcStockNew)
        {
            ...
            // Check Invoice Detail Exist or not

            string query2 = @"SELECT * FROM tblDCStoreFreeze WHERE InvoiceDetailId = " + aDcStockNew.InvoiceDetailId;
            DataTable aTable = aCommonInternalDal.DataContainerDataTable(query2, "SSIDB");
       ...
       "  ( DCStoreId , " +
        ...
        "     '" + aDcStockNew.ReceiveDate + "' , " +
        "    '" + aDcStockNew.ChalanNo + "' , " +
        "    '" + (aDcStockNew.ChalanDate ?? Convert.DBNull) + "', " +
        "    '" + aDcStockNew.ComUnitId + "' , " +
         "    '" + aDcStockNew.StockQty + "', " +
         "    '" + aDcStockNew.DamageQty + "' , " +
         "   '" + aDcStockNew.StockRcvDate + "' , " +
      ...
      "     )";
        ...
        public int UpdateInvoiceMasterDetail()
        {
            List<SqlParameter> aSqlParameterList = new List<SqlParameter>();
            //aSqlParameterList.Add(new SqlParameter("@OrderID", Invoice));
            return aCommonInternalDal.RunStoreProcedure("sp_ERROR_InvoiceMasterDetailAmount", aSqlParameterList, "SSIDB");
        }

        public int UpdateInvoiceDetailNotExist()
        {
            List<SqlParameter> aSqlParameterList = new List<SqlParameter>();
            //aSqlParameterList.Add(new SqlParameter("@OrderID", Invoice));
            return aCommonInternalDal.RunStoreProcedure("sp_ERROR_InvoiceDetailNotExistUpdate", aSqlParameterList, "SSIDB");
        }
 ...
 "  VALUES  ( '" + amountDao.CustomerId + "' , " +
      ...
      "     )";
        ...
        public DataTable CheckInvoiceDetailExistOrNot(string invoicenNo)
        {
            string query = @"SELECT INV.InvoiceId,OrderId FROM tblInvoice AS INV 
	                         LEFT JOIN tblInvoiceDetail AS IVD ON INV.InvoiceId = IVD.InvoiceId
	                         WHERE InvoiceDetailId IS NULL AND LTRIM(RTRIM(UPPER(InvoiceNo))) = LTRIM(RTRIM(UPPER('" + invoicenNo + "')))";
            return aCommonInternalDal.DataContainerDataTable(query, "SSIDB");
        }
...
where I.ComUnitId= '" + Dcid + "' and tblD.ManufacId='" + ManufId + "' and tblMarket.MarketId='" + market + "' and I.AreaCode='" + tr + "' and InvoiceDate='" + invDate + "' order by OrderNo";
        ...
        {
            string query = @"     SELECT  '"+ Code + @"' TopSheetGenCode,Ct.CustomerType CustomerType, CellNo,(C.Address + '[' +  Address +']')		 as Address	,'Route Name : ' + rt.RouteName as MarketName ,* 				
...
where I.InvoiceId in (" + invNoid + ")   order by I.InvoiceNo asc";
        ...
        {
            string query = @"  SELECT   'Route Name : ' +  rt.RouteName as MarketName,I.InvoiceDate,D.ProductCode,D.ProductName,sum(0) as BatchNo,D.PackSize,SUM(D.Quantity) AS Quantity  FROM dbo.tblInvoice I   with (nolock)
  INNER JOIN tblCustMaster C  with (nolock)  ON I.CustomerMasterId = C.CustomerMasterId  
   ...
   where I.InvoiceId in (" + invNoid + @")   
                          ...
                          GROUP BY rt.RouteName,I.InvoiceDate,D.ProductCode,D.ProductName ,D.PackSize ,rt.RouteName order by D.ProductName asc";
        
            return aCommonInternalDal.DataContainerDataTable(query, "SSIDB");
        }

        public DataTable InvoiceDetailDataForReportNew(string topsheetId, string Route, string invDat, string code)
        {
            ...
            try
            {
                ...
                List<SqlParameter> aSqlParameters = new List<SqlParameter>();
                aSqlParameters.Add(new SqlParameter("@TopsheetId", Convert.ToInt32(topsheetId)));
                aSqlParameters.Add(new SqlParameter("@Code", code));
                DataTable dt = accessManager.GetDataTable("sp_GET_TopSheetInvoiceDetailInfo", aSqlParameters);
...
//            return aCommonInternalDal.DataContainerDataTable(query, "SSIDB");
        ...
        public DataTable InvoiceMainDataForReportNew(Int32 topsheetId, string code)
        {
            try
            {
                accessManager.SqlConnectionOpen(DataBase.SalesDB);
                ...
                aSqlParameters.Add(new SqlParameter("@TopsheetId", Convert.ToInt32(topsheetId)));
                aSqlParameters.Add(new SqlParameter("@Code", code));
                DataTable dt = accessManager.GetDataTable("sp_GET_TopSheetInvoiceInfo", aSqlParameters);
        ...
        public DataTable GetTopSheetInfo(string topsheetId, string Route, string InvDate)
        {
            try
            {
                accessManager.SqlConnectionOpen(DataBase.SalesDB);
                ...
                List<SqlParameter> aSqlParameters = new List<SqlParameter>();
                aSqlParameters.Add(new SqlParameter("@TopsheetId", Convert.ToInt32(topsheetId)));
                DataTable dt = accessManager.GetDataTable("sp_GET_TopSheetReportInfo", aSqlParameters);
        ...
        public DataTable GetTopSheetWiseInvoice(string topSheetId, string empty, string s)
        {
            try
            {
                accessManager.SqlConnectionOpen(DataBase.SalesDB);
                ...
                List<SqlParameter> aSqlParameters = new List<SqlParameter>();
                aSqlParameters.Add(new SqlParameter("@TopsheetId", Convert.ToInt32(topSheetId)));
                DataTable dt = accessManager.GetDataTable("sp_GET_TopSheetWiseInvoiceNo", aSqlParameters);
        ...
        public DataTable GetCustomerPaymebtType(string customerMasterId)
        {
            try
            {
                accessManager.SqlConnectionOpen(DataBase.SalesDB);
                ...
                List<SqlParameter> aSqlParameters = new List<SqlParameter>();
                aSqlParameters.Add(new SqlParameter("@CustomerMasterId", Convert.ToInt32(customerMasterId)));
                DataTable dt = accessManager.GetDataTable("sp_GET_CustomerPaymentType", aSqlParameters);
        ...
        public void UpdateDeliveryStatus()
        {
            try
            {
                ...
                accessManager.UpdateData("sp_Error_DeliveryStatusNotUpdate", aSqlParameters);
        ...
        public void ProcessProbationalDiscount(string invoiceNo)
        {
            try
            {
                accessManager.SqlConnectionOpen(DataBase.SalesDB);
                ...
                List<SqlParameter> aSqlParameters = new List<SqlParameter>();
                aSqlParameters.Add(new SqlParameter("@InvoiceNo", invoiceNo));

                accessManager.UpdateData("sp_Canmpaign_DiscountProbationInsert", aSqlParameters);
        ...
        public bool SaveDataForInvoiceDetails(List<InvoiceDetail> aList)
        {
            bool status = false;
           
            foreach (var aDetail in aList)
            {

                List<SqlParameter> aSqlParameterList = new List<SqlParameter>();

                aSqlParameterList.Add(new SqlParameter("@InvoiceDetailId", aDetail.InvoiceDetailId));
                
              
                aSqlParameterList.Add(new SqlParameter("@TotalPrice", aDetail.TotalPrice));
                aSqlParameterList.Add(new SqlParameter("@DiscountPercentage", aDetail.DiscountPercentage));
                aSqlParameterList.Add(new SqlParameter("@DiscountAmount", aDetail.DiscountAmount));
                aSqlParameterList.Add(new SqlParameter("@TotalPriceVatAmount", aDetail.TotalPriceVatAmount));
                aSqlParameterList.Add(new SqlParameter("@NetAmount", aDetail.NetAmount));

                if (aCommonInternalDal.UpdateAction("sp_Update_InvoiceDetails_Manual", aSqlParameterList))
                {
                    status = true;
                }
        ...
        public DataTable LoadProductOrderDetails(string orderDetailsId)
        {
            string query = "SELECT OrderDetailId,ProductCode,Quantity,IsCampaignProduct,ISGiftProduct FROM tblOrderDetail WHERE OrderDetailId ='" + orderDetailsId + "'";
            return aCommonInternalDal.DataContainerDataTable(query, "SSIDB");
        }

        public DataTable CheckInvoiceExistOrNot(string invoicenNo)
        {
            string query = @"SELECT INV.InvoiceId,OrderId FROM tblInvoice AS INV 
	                         LEFT JOIN tblInvoiceDetail AS IVD ON INV.InvoiceId = IVD.InvoiceId
	                         WHERE LTRIM(RTRIM(UPPER(InvoiceNo))) = LTRIM(RTRIM(UPPER('" + invoicenNo + "')))";
            return aCommonInternalDal.DataContainerDataTable(query, "SSIDB");
        }
    }
